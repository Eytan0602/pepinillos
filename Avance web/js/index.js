...
function updateUserDisplay(user) {
  ...
  if (user && user.email) {
    // Si hay usuario logueado, mostrar su email
    if (sidebarUsername) {
      sidebarUsername.textContent = user.email;
    }
    
    // También actualizar etiqueta de usuario en el header
    if (userLabel) {
      userLabel.textContent = user.name || 'Usuario';
    }
  } else {
    // Si no hay usuario, mostrar placeholder
    if (sidebarUsername) {
      sidebarUsername.textContent = 'No has iniciado sesión';
    }
...
function verificarUsuario() {
  return localStorage.getItem('usuarioRegistrado') ? true : false;
}
